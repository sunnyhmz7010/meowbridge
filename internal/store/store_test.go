...
func TestBootstrapAllowsEmptyMeowAPIBaseURLForInitialSetting(t *testing.T) {
	ctx := context.Background()
	st, cleanup := openTestStore(t)
	defer cleanup()

	if err := st.Bootstrap(ctx, BootstrapOptions{
		AdminPassword:    "first-password",
		LogRetentionDays: 14,
	}); err != nil {
	...
	value, err := st.GetSetting(ctx, "meow_api_base_url")
	if err != nil {
		t.Fatalf("GetSetting(meow_api_base_url): %v", err)
	}
	if value != "" {
		t.Fatalf("meow_api_base_url = %q, want empty", value)
	}
}

...
func (s *Store) CreateEndpoint(ctx context.Context, input EndpointInput) (Endpoint, error) {
	...
	res, err := s.db.ExecContext(ctx, `
		...
		VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, input.Name, input.Token, input.MeowNickname, input.DefaultTitle, input.MsgType, input.HTMLHeight, input.DefaultURL, input.DefaultImgURL, boolToInt(input.Active), now, now)
	if err != nil {
		return Endpoint{}, err
	}

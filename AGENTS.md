...
- MeoW API 地址固定为代码内置值，不通过环境变量、管理后台或 Docker 参数暴露配置入口。
- Docker 部署只维护 `docker run` 示例；不再维护 Docker Compose 支持。
- 管理员密码默认通过首次打开 `/admin/` 页面设置；`ADMIN_PASSWORD` 仅作为可选无人值守初始化入口，不得在 README 的主部署命令中要求传入。
- 管理后台 UI 后续保持现代化、简约风格，并支持亮色模式、暗色模式和跟随系统。
- 管理后台视觉基线采用 Stitch 生成的 Terminal Precision 方向：固定侧栏、扁平卡片、清晰边框、紧凑表格、靛蓝主色、代码/JSON 区域保持终端风格；优先复用现有 Vue 组件和 class，不直接引入 Stitch 静态 HTML。
- Webhook 解析支持 endpoint 级 `parser_config`：未配置时走内置解析器链，配置后优先使用预设或自定义字段映射，仍需保留完整 JSON fallback。
...
- `ADMIN_PASSWORD`：可选无人值守首次初始化密码；未提供时首次打开管理后台页面设置。
- `HTTP_PORT`：HTTP 监听端口，默认 `8080`，值不带冒号。
- `JWT_SECRET`：JWT 签名密钥；未提供时首次启动自动生成并持久化到 SQLite。
- `LOG_RETENTION_DAYS`：日志保留天数，默认 `14`。
- `TZ`：容器时区示例使用 `Asia/Shanghai`；业务数据仍按 UTC 存储。
...
- 保留现有 Release / CD / Docker Publish 工作流拆分，不将其重构为单一发布工作流。
